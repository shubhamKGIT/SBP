...
def read_cihx_file(file_path):
    """reads the context in cihx file

    ARGS
    ----
        file_path: cihx file path

    RETURNS
    -------
        cihx_info: dict 
            dictionary of parameters from cihx file
    """
    try:
...
def read_mraw_file(file_path, frame_shape, dtype=np.uint16):
    ...
    ----
        file_path: str | Path
            path to mraw file
        frame_shape: used to pass frame shape, needed to read marwa file
            needs to know the frame shape as writen to the mraw file, (1024, 1024) for photron full screen
        dtype: np.units16 | np.uint12
            written at inteter 16, used to calculate the frame bit size

    RETURNS
    -------
        memmap_array: np.ndarray
            data from the mraw file read using np.memmap(path, dtype, mode, shape)
    """
    # Calculate the size of each frame in bytes
    frame_size = np.prod(frame_shape) * np.array([], dtype=dtype).itemsize
    # Calculate the number of frames in the file
    file_size = np.fromfile(file_path, dtype=np.int32, count=1)[0]
...
def process_experiment_data(experiment_number):
    ...
    ----
        experiment_number: int
            folder path is built based on exp number within proj_dir/data
